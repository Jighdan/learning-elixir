defmodule Rumbl.AccountsTest do
  ...
  describe "register_user/1" do
    ...
    test "with valid data inserts user" do
      ...
      assert [%User{id: ^id}] = Accounts.get_all()
    end
    ...
    test "does not accept long usernames" do
      attrs = Map.put(@valid_attrs, :username, String.duplicate("ab", 15))
  ...
  describe "authenticate_by_username_and_pass/2" do
    ...
    test "returns unauthorized error with invalid password", %{user: user} do
      assert {:error, :unauthorized} =
               Accounts.authenticate_by_username_and_pass(user.username, "baspass")
    end
